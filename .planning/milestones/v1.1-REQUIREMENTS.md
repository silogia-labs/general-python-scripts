# Requirements Archive: v1.1 Contract Hardening & Make.com Alignment
...
- [x] **VALI-01**: User can validate the emitted JSON payload against an in-repo JSON Schema by passing `--validate` to the CLI
- [x] **VALI-02**: When `--validate` is passed and the payload violates the schema, the CLI exits non-zero with a PII-safe stderr message naming the JSON Pointer path of the violation (no cell content leaked; T-PII-01 carry-forward)
- [x] **VALI-03**: The schema at `quizify-csv-to-json-webhook/docs/webhook-schema.json` validates envelope structure — contact fields by name and type, locked tail order via `required`, presence and well-formedness of `question-N`/`answers-N`/`answers-tags-N` triples via `patternProperties` — without constraining question text values
- [x] **VALI-04**: Validation is opt-in only — default behavior of all v1.0 CLI invocations is preserved when `--validate` is absent (no-flag callers see zero behavioral change)
- [x] **VALI-05**: `fastjsonschema` (>= 2.21.2) is an optional extra (`pip install '.[validate]'`); the CLI imports it lazily inside the validation function and prints an actionable stderr error if `--validate` is used without the extra installed
- [x] **VALI-06**: README documents the `--validate` flag, the `[validate]` extra installation step, and the schema path; the D-11 drift test (`test_readme_help_alignment.py`) passes after the additions

### Trailer Hardening (TRAIL-XX)

- [x] **TRAIL-01**: Scoring trio (`result-logic`, `score-category`, `score-value`) is extracted from trailer cells by canonical column-name lookup using NFC+casefold equality (replaces positional `trailer_cells_decoded[0..2]`; D-15 retired in favor of name-based binding)
- [x] **TRAIL-02**: When a canonical trailer column is missing from the CSV, the CLI emits an empty string for that scoring field and logs a PII-safe WARNING naming the absent column. Fallback to positional indexing is explicitly forbidden (per PITFALLS analysis: "the v1.0 bug wearing a different costume")
- [x] **TRAIL-03**: Default-order callers see no behavioral change; non-default `--trailer-columns` callers receive the bugfix automatically. The behavioral correction for non-default orders is documented in MILESTONES.md (under v1.1) as a bugfix
...
|-------------|-------|--------|
| VALI-01 | Phase 6 | Complete |
| VALI-02 | Phase 6 | Complete |
| VALI-03 | Phase 6 | Complete |
| VALI-04 | Phase 6 | Complete |
| VALI-05 | Phase 6 | Complete |
| VALI-06 | Phase 6 | Complete |
| TRAIL-01 | Phase 5 | Complete |
| TRAIL-02 | Phase 5 | Complete |
| TRAIL-03 | Phase 5 | Complete |
